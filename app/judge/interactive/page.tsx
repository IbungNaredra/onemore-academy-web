...
export default async function JudgeInteractivePage() {
  await requireJudge();
  const session = await auth();
  const email = session?.user?.email;
  if (!email) redirect("/login?callbackUrl=/judge/interactive");

  const q = await getJudgeQueue(email, ContentType.INTERACTIVE_CONTENT);
  ...
  return (
    <main className="panel-page judge-page judge-wide">
      <JudgeLaneNav active="interactive" />
      ...
      <JudgeQueueView q={q} returnPath="/judge/interactive" showContentTypeInCards={false} />
